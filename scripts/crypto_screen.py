#!/usr/bin/env python3
"""Momentum / relative strength screen over crypto universe. Prints ranks."""
...
def main():
    ...
    print(f"Downloading ~2y history for {len(tickers)} crypto tickers...")
    try:
        data = yf.download(
            tickers,
            period="2y",
    ...
    top = df.head(10)
    bottom = df.tail(min(5, len(df)))

    print("\n=== Crypto ranks by 63d total return ===")
    for _, r in top.iterrows():
        r252s = (
            "n/a"
            if pd.isna(r["ret_252d"]) or r["ret_252d"] is None
            else f"{100 * r['ret_252d']:.2f}%"
        )
        print(
            f"{int(r['rank']):2d}. {r['ticker']:10s}  63d={100 * r['ret_63d']:+.2f}%  "
            f"252d={r252s}  secondary_ok={r['secondary_ok']}  sector={r['sector']}"
        )

    print("\n=== Bottom by 63d total return ===")
        ...
        "rankings": [
            {
                ...
                "ret_252d": None
                if r["ret_252d"] is None
                or (isinstance(r["ret_252d"], float) and np.isnan(r["ret_252d"]))
                else float(r["ret_252d"]),
