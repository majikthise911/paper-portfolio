#!/usr/bin/env python3
"""Mark-to-market crypto paper sleeve vs BTC-USD. Does not trade."""
...
LEDGER = SLEEVE / "state" / "ledger.jsonl"
...
def main():
    ...
    benchmark = uni.get("benchmark", "BTC-USD")
    now = datetime.now(TZ)
    ...
    pf["btc_last_close"] = round(btc_px, 4) if btc_px is not None else None
    ...
    ledger_entry = {
        ...
        "btc_last_close": pf["btc_last_close"],
    ...
    if btc_px is not None:
        lines.append(
            f"BTC-USD last close: ${btc_px:,.2f}. With 0% invested, portfolio return is cash (0%). "
            "Versus BTC-USD is informational only until holdings are approved and funded."
        )
    else:
        lines.append("BTC-USD last close could not be fetched this run.")
    lines.extend(["", "## Positions", ""])
    if not marked:
        lines.append("No positions. Book is all cash.")
    else:
        lines.append("| Ticker | Qty | Last price | Market value | Stale |")
        lines.append("|--------|-----|------------|--------------|-------|")
        for t, m in sorted(marked.items()):
            lines.append(
                f"| {t} | {m['shares']} | ${m['last_price']:,.6f} | ${m['market_value']:,.2f} | {m['stale']} |"
            )
    if failed:
    ...
    print(f"btc_last_close={pf['btc_last_close']}")
